                               ...
                               struct buffer_head *bitmap_bh, unsigned long *count)
{
	...
	/* ? */
	first_free_bit = find_next_zero_bit_le(bitmap_bh->b_data, nblocks, 0);	// offset = 0 -> from the beginning
	if(first_free_bit >= nblocks) {
		printk(KERN_ERR "Did not find available block!");
		return -1;
	}
	for(num = 0; num < *count && first_free_bit + num < nblocks; ++num){
		if(ext2_set_bit_atomic(sb_bgl_lock(EXT2_SB(sb), group), first_free_bit + num, bitmap_bh->b_data) == 1){
			printk(KERN_ERR "Failed to set bit!");
			break;
		}
	}
	*count = num;
