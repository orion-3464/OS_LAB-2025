                             ...
                             struct folio **foliop)
{
	...
	for (i = 0; i < npages; i++) {
		...
		/* Get the i-th page of the directory */
		kaddr = ext2_get_folio(dir, i, 0, &folio);
		if (IS_ERR(kaddr))
			return ERR_CAST(kaddr);

		/* Get the last byte used in this page to specify the point the first byte that is not used */
		int last_byte = ext2_last_byte(dir, i);
		if (last_byte == 0) {
			folio_release_kmap(folio, kaddr);
			continue;
		}

		/* Scan through all entries in the page */
		de = (ext2_dirent *)kaddr;
		while ((char *)de < kaddr + last_byte) {
			if (de->rec_len < reclen || (char *)de + reclen > kaddr + last_byte) {
				/* Malformed directory entry, skip the rest of the page */
				folio_release_kmap(folio, kaddr);
				return ERR_PTR(-EIO);
			}
