                                         ...
                                         struct buffer_head **p)
{
	...
	*p = NULL;
	...
	if(!gdp) {
		printk(KERN_ERR "Unable to acceess group descriptor");
		goto einval;
	}
	...
	if(!bh) {
		printk(KERN_ERR "Unable to read block from disk");
		goto eio;
	} 

	*p=bh;
	return (struct ext2_inode *) bh->b_data + (offset * inode_sz) % blocksize;
